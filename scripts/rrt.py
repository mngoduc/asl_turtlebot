    ...
    def solve(self, eps, max_iters = 1000, goal_bias = 0.05):
        ...
        for i in range(1, max_iters):
            ...
            if self.is_free_motion(self.obstacles, x_near, x_new):
                ...
                P[n] = idx_near
                n += 1
                # reconstruct path once reached desired/goal
                if np.linalg.norm(x_new[:2] - self.x_goal[:2]) < 0.1:#(np.array_equal(x_new, self.x_goal)):
...
class DubinsRRT(RRT):
    ...
    def find_nearest(self, V, x):
        dist = np.apply_along_axis(self.distance, 1, V, x)
        return np.argmin(dist)
    ...
    def plot_path(self, V, resolution = np.pi/24, **kwargs):
        pts = []
        for i in range(len(V) - 1):#(V.shape[0] - 1):
