...
from asl_turtlebot.msg import DetectedPuddle
# look up doc.
...
class PuddleViz:
    def __init__(self):
        ...
        ###################################
        self.puddle_pos_pub = rospy.Publisher("/detector/puddle", DetectedPuddle, queue_size=10)
        ###################################
    ...
    def velodyne_callback(self, msg):
        ...
        # convert point to polar coords (r = ranges, theta = angles)
        ...
        # looking within a slice of field of view of robot
        #pts_within_angle = (np.abs(pt_angles) <= 2.*np.pi)
        pts_within_angle = (pt_angles < np.pi/6) & (pt_angles > -np.pi/6)

        # filtered points
        # points both within range and cone of view
        ...
        # if cluster of points exceeds threshold, characterize the puddle
        # print "number of filtered points: ", sum(filtered_points)
    ...
    def loop(self):
        ...
        if self.puddle_mean is not None:
            ...
            try:
                ...
                # make puddle marker
                x_width = np.max(np.abs(pt.point.x)) - np.min(np.abs(pt.point.x))
                y_width = np.max(np.abs(pt.point.y)) - np.min(np.abs(pt.point.y))
                # print "var x var y var z: ", self.puddle_var

                ellipse_points = compute_ellipse_points(np.max(np.abs(pt.point.y))/4, np.max(np.abs(pt.point.x))/4)
                ...
                ############################
                dist = np.hypot(self.puddle_mean[0], self.puddle_mean[1])
                cl = 0
                # publishes detected puddle and its Distance
                object_msg = DetectedPuddle()
                object_msg.id = cl
                object_msg.distance = dist
                object_msg.mean = self.puddle_mean
                object_msg.variance = self.puddle_var
                self.puddle_pos_pub[cl].publish(object_msg)
                ################################
                ################################
